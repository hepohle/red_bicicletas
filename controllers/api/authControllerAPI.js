...
module.exports = {
    authenticate: function(req, res, next) {
        Usuario.findOne ({email:require.body.email}, function(err, userInfo){
            if(err){
                ...
                if (userInfo === null){ return require.status(401).json({status:'error', message: 'Invalido email/password', data:null}); }
    ...
    forgotPassword: function(req, res, next) {
        Usuario.findOne({email: req.body.email}, function(err, usuario){
            if(!usuario) return req.status(401).json({message: 'No existe el usuario', data:null});
